    ...
    public static void generate() {
        NotEnoughChest.WOOD_TYPES.forEach(wt->{
            if (!wt.getNamespace().equals("minecraft")) {
                String t = wt.getPath();
                itemFromParent(t + "_chest", MODID + ":item/chest_base", new TextureKey("wood_type", MODID + ":entity/chest/" + t));
                blockFromParent(t + "_chest", "minecraft:block/chest", new TextureKey("particle", MODID + ":entity/chest/" + t));
            }
        });
    }
